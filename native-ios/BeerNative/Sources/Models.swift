...
struct PasskeyRegisterOptionsResponse: Decodable {
    ...
    enum CodingKeys: String, CodingKey {
        case ok, error, challenge, label, publicKey
        case rpId = "rp_id"
        case userId = "user_id"
    ...
    var resolvedChallenge: String {
        if let publicKey { return publicKey.challenge }
        return challenge ?? ""
    }

    var resolvedRpId: String {
        if let id = publicKey?.rp?.id { return id }
        return rpId ?? "eiter.freeboxos.fr"
    }

    var resolvedUserId: String {
        if let id = publicKey?.user.id { return id }
        return userId ?? ""
    }

    var resolvedUserName: String {
        if let name = publicKey?.user.name { return name }
        return userName ?? ""
    }
}
...
struct PasskeyLoginOptionsResponse: Decodable {
    ...
    enum CodingKeys: String, CodingKey {
        case ok, error, challenge, publicKey
        case rpId = "rp_id"
    ...
    var resolvedRpId: String {
        if let id = publicKey?.rpId { return id }
        return rpId ?? "eiter.freeboxos.fr"
    }

    var resolvedAllowCredentials: [PasskeyAllowCredential]? {
        if let creds = publicKey?.allowCredentials { return creds }
